const MarkBook = {
    ...
    parseRecipe(text) {
        ...
        else if (/x\s*\/\s*31|entre\s*31|por\s*31/.test(t)) rec.formula = "X / 31";
        else if (/x\s*\/\s*32|entre\s*32/.test(t)) rec.formula = "X / 32";
        else if (/x\s*\/\s*2|entre\s*2/.test(t) && !/x\s*\*\s*2/.test(t)) rec.formula = "X / 2";
        ...
        else if (/\bbcd\b/.test(t)) rec.formula = "BCD";
        else if (/x\s*1000|metros|por 1000/.test(t)) rec.formula = "X * 1000";
        else if (/x\s*100|por 100/.test(t)) rec.formula = "X * 100";
        else if (/x\s*\*\s*64|x\s*64/.test(t)) rec.formula = "X * 64";
        else if (/x\s*\*\s*31/.test(t)) rec.formula = "X * 31";
        else if (/x\s*\*\s*32/.test(t)) rec.formula = "X * 32";
        else if (/x\s*16/.test(t)) rec.formula = "X * 16";
        else if (/x\s*10|por\s*10|por diez|multiplicad|veces 10|x10/.test(t)) rec.formula = "X * 10";
        else if (/xorbytes\s+([0-9a-f]{2}(?:-[0-9a-f]{2})+)/i.test(raw)) rec.formula = "X XORBYTES " + raw.match(/xorbytes\s+([0-9A-Fa-f]{2}(?:-[0-9A-Fa-f]{2})+)/i)[1].toUpperCase();
        else if (/xorbyte\s+([0-9a-f]{1,2})/i.test(raw)) rec.formula = "X XORBYTE " + raw.match(/xorbyte\s+([0-9A-Fa-f]{1,2})/i)[1].toUpperCase();
        ...
        else if (/crc16|ccitt/.test(t)) rec.chk = "CRC16";
